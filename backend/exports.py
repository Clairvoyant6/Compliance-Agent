...
def report_to_pdf(report: dict) -> bytes:
    ...
    story.append(Paragraph(
        f"{escape(str(biz.get('name', '—')))} &nbsp;·&nbsp; {escape(str(biz.get('gstin', '—')))} &nbsp;·&nbsp; {escape(str(biz.get('period', '—')))}",
        muted,
    ))
    ...
    story.append(Paragraph(
        ...
        f"<i>({escape(str(net.get('status', '—')))})</i>", body,
    ))
    story.append(Paragraph(
    ...
    story.append(Paragraph(
        ...
        f"<b>Status:</b> {escape(str(hc.get('overall_status', '—')))}", body,
    ))
    ...
    if analysis:
        ...
        story.append(Paragraph(f"<b>Risk level:</b> {escape(str(analysis.get('risk_level', '—')))}", body))
        if analysis.get("period_summary"):
            story.append(Paragraph(escape(str(analysis["period_summary"])), body))
        if analysis.get("key_issues"):
            for k in analysis["key_issues"]:
                story.append(Paragraph(f"• {escape(str(k))}", body))
