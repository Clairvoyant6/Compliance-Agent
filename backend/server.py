...
def run_full_analysis(dataset_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(500, f"Agent pipeline error: {e}")
...
def send_test_email(body: TestEmailIn, db: Session = Depends(get_db)):
    html = "<p>This is a test email from <b>ComplianceIQ</b>. If you received this, notifications are working.</p>"
    result = send_email(body.to, "ComplianceIQ test email", html)
